...
_LLAVA_DIR = Path(__file__).resolve().parents[1]
_DEFAULT_PRED = _LLAVA_DIR / "evaluate" / "results" / "vqav2_random10.jsonl"
_DEFAULT_ANN = _LLAVA_DIR / "Benchmark" / "VQAv2" / "Annotations" / "v2_mscoco_val2014_annotations.json"
_DEFAULT_OUT = _LLAVA_DIR / "evaluate" / "results" / "vqav2_random10_scored.json"
...
def main():
    parser = argparse.ArgumentParser(description="Score VQAv2 subset predictions with official soft accuracy")
    parser.add_argument("--pred-jsonl", type=str, default=str(_DEFAULT_PRED), help="Path to predictions JSONL from evaluate_vqav2_random.py")
    parser.add_argument("--annotations-json", type=str, default=str(_DEFAULT_ANN), help="Path to v2_mscoco_val2014_annotations.json")
