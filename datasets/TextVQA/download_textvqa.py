...
# 设置缓存目录
os.environ['HF_HOME'] = '/data/model/Inference_VLM/VLM_Infra/datasets/TextVQA/hf_cache'
...
def download_textvqa():
    ...
    try:
        ...
        if dataset is not None:
            ...
            # 检查数据文件位置
            data_file = '/data/model/Inference_VLM/VLM_Infra/datasets/TextVQA/TextVQA_VAL.tsv'
...
if __name__ == "__main__":
    print("开始下载TextVQA benchmark数据集...")
    
    success = download_textvqa()
    
    if success:
        print("\n🎉 TextVQA benchmark数据集下载完成！")
        print("📁 数据位置: /data/model/Inference_VLM/VLM_Infra/datasets/TextVQA/")
        print("📊 数据集信息: TextVQA_VAL (VQA类型, IMAGE模态)")
    else:
        print("\n❌ TextVQA benchmark数据集下载失败")
